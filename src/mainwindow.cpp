...
QString get_uuid(void)
{
    ...
    QString  now = QString::number((QDateTime::currentDateTime().currentMSecsSinceEpoch()-1296215058260LL));
...
QMap<QString, QString> MainWindow::get_todos_ordered_by_uuid()
{
    ...
    foreach (QString str, map_todo.keys())
    {
        ...
        if (ui->filter->text()=="")
        {
            ui->date_filter->setStyleSheet(QString::fromUtf8("background-color: rgb(230, 100, 100);"));
            ui->filter->setStyleSheet(QString::fromUtf8(""));
            if (QDateTime(todo.date, todo.time) <= QDateTime(ui->date_filter->date(), QTime::currentTime()))
            {
                QString index = QString::number(todo.priority) +  "|" + todo.date.toString() + "|" + todo.uuid;
                order_uuid[index] = todo.uuid;
            }
        }
        else
        {
            ui->filter->setStyleSheet(QString::fromUtf8("background-color: rgb(230, 100, 100);"));
...
void MainWindow::order()
{
    ...
    foreach (QString str, order_uuid.keys())
    {
        ...
        if (uuid != "")
        {
            ...
            update_item(ui->todo_list->item(ui->todo_list->count()-1), map_todo[uuid]);
...
{
    if (ui->todo_list->currentRow() == -1)     return "";
...
void MainWindow::on_todo_list_currentRowChanged(int currentRow)
{
    ...
    fill_todo(map_todo.value(ui->todo_list->item(currentRow)->data(Qt::UserRole).toString()));
    //ui->subject->setFocus();
...
{
    if (ui->text->document()->isModified()  == false)  return;
